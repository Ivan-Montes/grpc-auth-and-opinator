...
class DeleteVoteCommandHandlerTest {
	...
	@InjectMocks
	private DeleteVoteCommandHandler deleteVoteCommandHandler;

	private Event event;
	private DeleteVoteCommand deleteCommand;
	private Vote vote;
	...
	private void setUp() {	
		...
		deleteCommand = new DeleteVoteCommand(voteId);		
	...
	void handle_shouldReturnEvent() {
		...
		StepVerifier
		.create(deleteVoteCommandHandler.handle(deleteCommand))
		.expectNext(event)
		.verifyComplete();
