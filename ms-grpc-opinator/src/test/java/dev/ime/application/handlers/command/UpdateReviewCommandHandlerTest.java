...
class UpdateReviewCommandHandlerTest {
	...
	private void setUp() {	
		...
		updateCommand = new UpdateReviewCommand(reviewId, reviewText, rating);		
	...
	void handle_shouldReturnEvent() {
		...
		StepVerifier
		.create(updateReviewCommandHandler.handle(updateCommand))
		.expectNext(event)
		.verifyComplete();
