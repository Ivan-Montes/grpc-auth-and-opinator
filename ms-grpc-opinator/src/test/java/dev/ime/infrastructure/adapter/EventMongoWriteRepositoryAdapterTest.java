...
class EventMongoWriteRepositoryAdapterTest {
	...
	void save_WithEvent_ReturnMonoEvent() {
		...
		Mockito.when(eventMapper.fromEventDomainToEventMongo(Mockito.any(Event.class))).thenReturn(eventMongoEntity);
		Mockito.when(reactiveMongoTemplate.save(Mockito.any(EventMongoEntity.class))).thenReturn(Mono.just(eventMongoEntity));
		Mockito.when(eventMapper.fromEventMongoToEventDomain(Mockito.any(EventMongoEntity.class))).thenReturn(event);
        ...
        .verifyComplete();		
		Mockito.verify(eventMapper).fromEventDomainToEventMongo(event);
        Mockito.verify(reactiveMongoTemplate).save(eventMongoEntity);
        Mockito.verify(eventMapper).fromEventMongoToEventDomain(eventMongoEntity);
	}
