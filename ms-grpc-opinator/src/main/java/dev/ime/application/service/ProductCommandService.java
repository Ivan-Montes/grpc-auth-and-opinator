...
@Service
public class ProductCommandService implements CommandServicePort<ProductDto> {
	...
	public ProductCommandService(@Qualifier("productCommandDispatcher")CommandDispatcher commandDispatcher, PublisherPort publisherPort) {
		super();
	...
	@Override
	public Mono<Event> create(ProductDto dto) {

		return Mono.justOrEmpty(dto)
				.switchIfEmpty(Mono.error(new ResourceNotFoundException(Map.of(GlobalConstants.PROD_CAT, GlobalConstants.MSG_REQUIRED))))						
				.map(item -> new CreateProductCommand(UUID.randomUUID(), dto.productName(), dto.productDescription(), dto.categoryId()))
				.flatMap(this::runHandler)
				.flatMap(this::processEvents);	
	}

	@Override
	public Mono<Event> update(ProductDto dto) {

		return Mono.justOrEmpty(dto)
				.switchIfEmpty(Mono.error(new ResourceNotFoundException(Map.of(GlobalConstants.PROD_CAT, GlobalConstants.MSG_REQUIRED))))						
				.map(item -> new UpdateProductCommand(dto.productId(), dto.productName(), dto.productDescription(), dto.categoryId()))
				.flatMap(this::runHandler)
				.flatMap(this::processEvents);	
	}
	...
	public Mono<Event> deleteById(UUID id) {
		...
		return Mono.justOrEmpty(id)
				.switchIfEmpty(Mono.error(new ResourceNotFoundException(Map.of(GlobalConstants.PROD_CAT, GlobalConstants.MSG_REQUIRED))))						
				.map(item -> new DeleteProductCommand(id))
				.flatMap(this::runHandler)
				.flatMap(this::processEvents);	
	}
