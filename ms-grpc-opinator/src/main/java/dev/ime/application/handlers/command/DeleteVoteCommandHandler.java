...
@Component
public class DeleteVoteCommandHandler implements CommandHandler {
	...
	public DeleteVoteCommandHandler(EventWriteRepositoryPort eventWriteRepository,
	...
	public Mono<Event> handle(Command command) {
		...
		return Mono.justOrEmpty(command)
				.ofType(DeleteVoteCommand.class)
				.flatMap(this::validateDelete)
				.map(this::createEvent)
				.flatMap(eventWriteRepository::save)
				.switchIfEmpty(Mono.error(new CreateEventException(Map.of(GlobalConstants.DELETE_VOT, ""))));	
	}

	private Mono<DeleteVoteCommand> validateDelete(DeleteVoteCommand command) {
	...
	private Mono<Vote> validateIdExists(DeleteVoteCommand command) {
	...
	private Event createEvent(Command command) {		
		return eventMapper.createEvent(GlobalConstants.VOT_CAT, GlobalConstants.VOT_DELETED, command);
	}	
