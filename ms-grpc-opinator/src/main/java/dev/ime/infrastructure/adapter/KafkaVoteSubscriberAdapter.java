...
@Component
public class KafkaVoteSubscriberAdapter implements SubscriberPort {
	...
	private static final Logger logger = LoggerFactory.getLogger(KafkaVoteSubscriberAdapter.class);

	public KafkaVoteSubscriberAdapter(@Qualifier("voteProjectorAdapter")BaseProjectorPort baseProjector, @Qualifier("voteProjectorAdapter")ExtendedProjectorPort extendedProjector
			) {
		super();
	...
	private Map<String, Function<Event, Mono<?>>> initializeActionsMap() {
		...
		return Map.of(
				GlobalConstants.VOT_CREATED, baseProjector::create, 
				GlobalConstants.VOT_UPDATED, extendedProjector::update, 
				GlobalConstants.VOT_DELETED, baseProjector::deleteById
				);
	}

	@KafkaListener(topics = {GlobalConstants.VOT_CREATED, GlobalConstants.VOT_UPDATED, GlobalConstants.VOT_DELETED}, groupId = "msgrpcopinator-consumer-vote")
	@Override
