...
@Component
public class KafkaCategorySubscriberAdapter implements SubscriberPort {
	...
	private static final Logger logger = LoggerFactory.getLogger(KafkaCategorySubscriberAdapter.class);
	
	public KafkaCategorySubscriberAdapter(@Qualifier("categoryProjectorAdapter")BaseProjectorPort baseProjector, @Qualifier("categoryProjectorAdapter")ExtendedProjectorPort extendedProjector
			) {
		super();
	...
	private Map<String, Function<Event, Mono<?>>> initializeActionsMap() {
		...
		return Map.of(
				GlobalConstants.CAT_CREATED, baseProjector::create, 
				GlobalConstants.CAT_UPDATED, extendedProjector::update, 
				GlobalConstants.CAT_DELETED, baseProjector::deleteById
				);
	}

	@KafkaListener(topics = {GlobalConstants.CAT_CREATED, GlobalConstants.CAT_UPDATED, GlobalConstants.CAT_DELETED}, groupId = "msgrpcopinator-consumer-category")
	@Override
