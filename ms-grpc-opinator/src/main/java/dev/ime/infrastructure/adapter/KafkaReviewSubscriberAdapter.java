...
@Component
public class KafkaReviewSubscriberAdapter implements SubscriberPort {
	...
	private static final Logger logger = LoggerFactory.getLogger(KafkaReviewSubscriberAdapter.class);

	public KafkaReviewSubscriberAdapter(@Qualifier("reviewProjectorAdapter")BaseProjectorPort baseProjector, @Qualifier("reviewProjectorAdapter")ExtendedProjectorPort extendedProjector
			) {
		super();
	...
	private Map<String, Function<Event, Mono<?>>> initializeActionsMap() {
		...
		return Map.of(
				GlobalConstants.REV_CREATED, baseProjector::create, 
				GlobalConstants.REV_UPDATED, extendedProjector::update, 
				GlobalConstants.REV_DELETED, baseProjector::deleteById
				);
	}

	@KafkaListener(topics = {GlobalConstants.REV_CREATED, GlobalConstants.REV_UPDATED, GlobalConstants.REV_DELETED}, groupId = "msgrpcopinator-consumer-review")
	@Override
