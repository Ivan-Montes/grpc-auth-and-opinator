...
@Component
public class KafkaUserAppSubscriberAdapter implements SubscriberPort {
	...
	private static final Logger logger = LoggerFactory.getLogger(KafkaUserAppSubscriberAdapter.class);

	public KafkaUserAppSubscriberAdapter(@Qualifier("userAppProjectorAdapter")BaseProjectorPort baseProjector, @Qualifier("userAppProjectorAdapter")ExtendedProjectorPort extendedProjector) {
		super();
	...
	private Map<String, Function<Event, Mono<?>>> initializeActionsMap() {
		...
		return Map.of(
				GlobalConstants.USERAPP_CREATED, baseProjector::create, 
				GlobalConstants.USERAPP_UPDATED, extendedProjector::update, 
				GlobalConstants.USERAPP_DELETED, baseProjector::deleteById
				);
	}

	@KafkaListener(topics = {GlobalConstants.USERAPP_CREATED, GlobalConstants.USERAPP_UPDATED, GlobalConstants.USERAPP_DELETED}, groupId = "msgrpcuserapp-consumer-userapp")
	@Override
	public Mono<Void> onMessage(ConsumerRecord<String, Event> consumerRecord) {
		...
		return Mono.just(consumerRecord)
				...
				.onErrorResume( error ->  Mono.fromRunnable(()-> logger.error(GlobalConstants.MSG_PATTERN_SEVERE, GlobalConstants.EX_BASIC, error.getMessage() != null? error.getMessage():GlobalConstants.EX_PLAIN)))
				.then();		
	}
